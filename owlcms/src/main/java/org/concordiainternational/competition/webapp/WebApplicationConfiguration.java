...
public class WebApplicationConfiguration implements HbnSessionManager, ServletContextListener {
    ...
    public static void insertInitialData(int liftersToLoad, org.hibernate.Session sess, boolean testMode) {
        if (sess.createCriteria(CompetitionSession.class).list().size() == 0) {
            ...
            competition.setFederationAddress(Messages.getString("Competition.defaultFederationAddress", Locale.getDefault())); //$NON-NLS-1$
            competition.setFederationEMail(Messages
                    .getString("Competition.defaultFederationEMail", Locale.getDefault())); //$NON-NLS-1$
            competition.setFederationWebSite(Messages.getString("Competition.defaultFederationWebSite", Locale.getDefault())); //$NON-NLS-1$
    ...
    protected static void setupEmptyCompetition(Competition competition, org.hibernate.Session sess) {
        ...
        File templateFile;
        URL templateUrl = platform1.getClass().getResource(
                "/templates/competitionBook/CompetitionBook_Total_" + getDefaultLanguage() + ".xls");
        try {
