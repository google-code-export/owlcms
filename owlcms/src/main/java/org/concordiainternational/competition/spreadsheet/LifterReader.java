...
public class LifterReader {
    final private static Logger logger = LoggerFactory.getLogger(LifterReader.class);

    // constants
    final static int START_ROW = 7;
    private static final int GENDER_COLUMN = 4;
    private static final int BODY_WEIGHT_COLUMN = 6;

    private CategoryLookup categoryLookup;
    private CompetitionSessionLookup competitionSessionLookup;
    private CategoryLookupByName categoryLookupByName;

    LifterReader(HbnSessionManager hbnSessionManager) {
    ...
    private String getString(WorkSheetHandle sheet, int row, int column) throws CellNotFoundException {
        CellHandle cell = sheet.getCell(row, column);
    ...
    Lifter readLifter(WorkSheetHandle sheet, int lifterNumber) {
        ...
        try {
            ...
            lifter.setSnatch1Declaration(getString(sheet, row, 9)); // not
                                                                    // normally
                                                                    // read, for
                                                                    // testing
            ...
            lifter.setCleanJerk1Declaration(getString(sheet, row, 14)); // not
                                                                        // normally
                                                                        // read,
                                                                        // for
                                                                        // testing
            ...
            logger.debug(LifterReader.toString(lifter, false));
            return lifter;
    ...
    private Category getCategory(WorkSheetHandle sheet, int row, int column) throws CellNotFoundException {
     ...
     * @throws CellNotFoundException
     */
    private String getGender(WorkSheetHandle sheet, int row, int column) throws CellNotFoundException {
    ...
    private CompetitionSession getCompetitionSession(WorkSheetHandle sheet, int row, int column) throws CellNotFoundException {
